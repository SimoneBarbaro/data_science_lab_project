...
from src.data.read_data import get_spider_data, get_old_tiger_data, get_tiger_data, dirname, \
    ORIGINAL_TIGER_DATA, FILTERED_SPIDER_DATA, FILTERED_TIGER_DATA  # , PROCESSED_SPIDER_DATA, PROCESSED_TIGER_DATA
...
def clean_spider_tiger_data():
    ...
    spider[spider.index.isin(tiger.index)].to_csv(os.path.join(dirname, "../../data", FILTERED_SPIDER_DATA), index=True, header=True)
    tiger[tiger.index.isin(spider.index)].to_csv(os.path.join(dirname, "../../data", FILTERED_TIGER_DATA), index=True, header=True)
