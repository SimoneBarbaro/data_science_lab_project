...
if __name__ == "__main__":
    ...
    # python ./statistical_analysis.py --run_name kmeans16 --level hlgt --method ranks --alpha 0.005 --print_only
    ...
    if args.method == "ranks":
        for term in ranked.iloc[:,1].unique():  # scores.iloc[:,1] corresponds to soc_term/pt_term/etc.
            ...
            significant = significant.append(ranked[(ranked.iloc[:,1] == term) & (ranked["rank"].isin(sig_values))])
        ...
        for term in ranked.iloc[:,1].unique():  # scores.iloc[:,1] corresponds to soc_term/pt_term/etc.
            ...
            significant = significant.append(ranked[(ranked.iloc[:,1] == term) & (ranked["tfidf_score"].isin(sig_values))])
